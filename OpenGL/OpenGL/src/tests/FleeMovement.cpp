...
#include "pathing/Fleeing.h"


namespace test {
	
	FleeMovement::FleeMovement()
        : m_Proj(glm::ortho(0.0f, 960.0f, 0.0f, 540.0f, -1.0f, 1.0f)), m_View(glm::translate(glm::mat4(1.0f), glm::vec3(0, 0, 0))), m_TranslationA(200, 200, 0), m_TranslationB(400, 200, 0), m_LastMove(0, 0, 0)
	{
	...
	void FleeMovement::OnUpdate(float deltaTime, double x_Mouse, double y_Mouse)
	{
        Flee(&m_TranslationA, x_Mouse, y_Mouse, &m_LastMove);
	}

	void FleeMovement::OnRender()
	{
        Renderer renderer;
        m_Texture->Bind();
	...
	void FleeMovement::OnImGuiRender()
	{
